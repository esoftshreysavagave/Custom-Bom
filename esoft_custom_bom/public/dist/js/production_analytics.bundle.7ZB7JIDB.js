(() => {
  ...
  // ../esoft_custom_bom/esoft_custom_bom/esoft_custom_bom/page/production_analytics/production_analytics.js
  frappe.pages["production-analytics"].on_page_load = function(wrapper) {
    ...
    $wrap.html(`
        ...
        <div id="tab-content-plan" class="pd-tab-pane active">
            ...
            <div class="pd-kpi loading" id="pkpi-active"><div class="pd-kpi-icon">\u{1F3ED}</div><div class="pd-kpi-val">\u2014</div><div class="pd-kpi-label">Active Plans</div></div>
            <div class="pd-kpi loading" id="pkpi-planned"><div class="pd-kpi-icon">\u{1F4E6}</div><div class="pd-kpi-val">\u2014</div><div class="pd-kpi-label">Planned Qty</div></div>
            ...
            <div class="pd-kpi loading" id="pkpi-pct"><div class="pd-kpi-icon">\u{1F4CA}</div><div class="pd-kpi-val">\u2014</div><div class="pd-kpi-label">Completion %</div></div>
            ...
            <div class="pd-kpi loading" id="pkpi-mrs"><div class="pd-kpi-icon">\u{1F4DD}</div><div class="pd-kpi-val">\u2014</div><div class="pd-kpi-label">Open MRs</div></div>
        </div>
    ...
    let _filters = {};
    let _jc_table_filter = {};
    let _item_highlight = null;
    ...
    function set_loading(sel) {
      $(sel).html('<div class="pd-spinner"></div>');
    }
    ...
    function load_item_bar() {
      ...
      frappe.call({ method: API + "get_item_wise_planned_vs_produced", args: _filters, callback: (r) => {
        ...
        if (!rows.length) {
          $("#chart-item-bar").html('<div class="pd-empty">No data</div>');
          return;
        }
          ...
          el.addEventListener("click", () => {
            setTimeout(() => {
              ...
              if (active) {
                ...
                (_a = $("#tbl-item-wise-body .pd-row-highlight")[0]) == null ? void 0 : _a.scrollIntoView({ behavior: "smooth", block: "nearest" });
              }
            }, 100);
          });
      } });
    ...
    function load_plan_donut() {
      ...
      frappe.call({ method: API + "get_chart_plan_status_distribution", args: _filters, callback: (r) => {
        ...
        let rows = r.message || [];
        if (!rows.length) {
          $("#chart-plan-donut").html('<div class="pd-empty">No data</div>');
          return;
        }
    ...
    function load_trend_line() {
      ...
      frappe.call({ method: API + "get_chart_production_trend", args: _filters, callback: (r) => {
        ...
        let rows = r.message || [];
        if (!rows.length) {
          $("#chart-trend-line").html('<div class="pd-empty">No data</div>');
          return;
        }
    ...
    function load_ws_bar() {
      ...
      frappe.call({ method: API + "get_chart_workstation_load", args: _filters, callback: (r) => {
        ...
        let rows = r.message || [];
        if (!rows.length) {
          $("#chart-ws-bar").html('<div class="pd-empty">No data</div>');
          return;
        }
        ...
        let datasets = st_set.map((s) => ({
          name: s,
          values: ws_set.map((w) => {
            let f = rows.find((x) => x.workstation === w && x.status === s);
            return f ? f.jc_count : 0;
          })
        }));
    ...
    function load_jc_donut() {
      ...
      frappe.call({ method: API + "get_chart_jobcard_status_distribution", args: _filters, callback: (r) => {
        ...
        let rows = r.message || [];
        if (!rows.length) {
          $("#chart-jc-donut").html('<div class="pd-empty">No data</div>');
          return;
        }
    ...
    function filter_delayed_table() {
      $("#tbl-delayed-jc-body tr").each(function() {
        ...
        let show = true;
        if (_jc_table_filter.status && jc_status !== _jc_table_filter.status)
          show = false;
        if (_jc_table_filter.workstation && jc_ws !== _jc_table_filter.workstation)
          show = false;
        $(this).toggle(show);
      });
      let visible = $("#tbl-delayed-jc-body tr:visible").length;
      let parts = [];
      if (_jc_table_filter.status)
        parts.push(_jc_table_filter.status);
      if (_jc_table_filter.workstation)
        parts.push(_jc_table_filter.workstation);
    ...
    function load_throughput_line() {
      ...
      frappe.call({ method: API + "get_chart_jobcard_throughput", args: __spreadProps(__spreadValues({}, _filters), { granularity: "day" }), callback: (r) => {
        destroy_chart("throughput");
        $("#chart-throughput-line").empty();
        let rows = r.message && r.message.data || [];
        if (!rows.length) {
          $("#chart-throughput-line").html('<div class="pd-empty">No data</div>');
          return;
        }
    ...
    function load_ws_util() {
      ...
      frappe.call({ method: API + "get_workstation_utilization", args: _filters, callback: (r) => {
        ...
        let rows = r.message || [];
        if (!rows.length) {
          $("#chart-ws-util").html('<div class="pd-empty">No time log data</div>');
          return;
        }
    ...
    function status_cls(s) {
      if (s === "Completed")
        return "pd-badge-green";
      if (s === "Work In Progress")
        return "pd-badge-blue";
      if (s === "On Hold")
        return "pd-badge-orange";
    ...
    function load_delayed_jc() {
      frappe.call({ method: API + "get_delayed_job_cards", args: _filters, callback: (r) => {
        ...
        let tbody = rows.map((x) => `
                ...
                </tr>`).join("") || '<tr><td colspan="8" class="pd-empty">No delayed job cards \u{1F389}</td></tr>';
        $("#tbl-delayed-jc-body").html(tbody);
      } });
    }
    function prog(pct) {
      if (pct === null || pct === void 0 || isNaN(pct))
        pct = 0;
    ...
    function load_drilldown() {
      ...
      frappe.call({ method: API + "get_plan_workorder_jobcard_drilldown", args: _filters, callback: (r) => {
        let plans = r.message || [];
        if (!plans.length) {
          $("#drilldown-wrap").html('<div class="pd-empty">No data found</div>');
          return;
        }
                ...
                return `
                                <div class="pd-node pd-node-mat">
                                    <div class="pd-node-header" style="cursor:default">
                                        <div class="pd-node-title">
                                            <span class="pd-icon">\u{1F529}</span>
            ...
            return `
                        <div class="pd-node pd-node-item">
                            <div class="pd-node-header" onclick="pd_toggle('wog-${safe_it}')">
                                <div class="pd-node-title">
                                    <span class="pd-icon">\u{1F4E6}</span>
          ...
          return `
                    <div class="pd-node pd-node-plan">
                        <div class="pd-node-header" onclick="pd_toggle('itg-${safe_plan}')">
                            <div class="pd-node-title">
                                <span class="pd-icon">\u{1F4CB}</span>
    ...
    function load_from_url() {
      ...
      if (p.get("from_date"))
        f_from.set_value(p.get("from_date"));
      if (p.get("to_date"))
        f_to.set_value(p.get("to_date"));
      if (p.get("warehouse"))
        f_wh.set_value(p.get("warehouse"));
      if (p.get("workstation"))
        f_ws.set_value(p.get("workstation"));
      if (p.get("status"))
        f_st.set_value(p.get("status"));
      if (p.get("production_plan"))
        f_pp.set_value(p.get("production_plan"));
    }
    load_from_url();
    refresh_all();
    start_auto_refresh();
  ...
  function inject_prod_styles() {
    if (document.getElementById("prod-dash-styles"))
      return;
...
//# sourceMappingURL=production_analytics.bundle.7ZB7JIDB.js.map
